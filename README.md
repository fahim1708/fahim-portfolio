...
Set `DJANGO_DEBUG=False`, provide a strong `DJANGO_SECRET_KEY`, configure `DJANGO_ALLOWED_HOSTS`, set `RENDER_EXTERNAL_HOSTNAME`, and set `DATABASE_URL` to your PostgreSQL connection string. Configure the three Cloudinary variables for persistent admin-uploaded media. Static files are collected into `staticfiles/` and served by WhiteNoise; media files are stored separately in Cloudinary. Render can use the included `render.yaml` build and start commands.
